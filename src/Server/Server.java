...
import Lib.*;
import Lib.Exceptions.ClosingSocketException;
...
public class Server {
    ...
    public void run() throws InterruptedException {
        ...
        try {
           ...
           /* openServerSocket();
            ...
            MainServer.logger.warning("Сервер не может быть запущен!");
    ...
    private void acceptt(SelectionKey selectionKey) throws IOException {
        ...
        try {
            ...
            MainServer.logger.warning("Unable to accept channel");
            e.printStackTrace();
            selectionKey.cancel();
        }
        if (channel != null) {
    ...
    private void read(SelectionKey selectionKey) throws IOException {
        ...
        try {
            ...
            MainServer.logger.warning("IOException " + socketChannel);
            selectionKey.cancel();


        } catch (TimeoutException e) {
            MainConsole.println("Disconnect " + socketChannel);
            MainServer.logger.warning("Disconnect " + socketChannel);
            selectionKey.cancel();
        }
        if (cmd != null) {
